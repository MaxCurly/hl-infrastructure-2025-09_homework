...
Target_PGs_per_OSD- целевое кол-во pg на osd (для малых кластеров взято значение 100)
OSD_#- кол-во osd в кластере
%Data- процент хранилища, занимаемый пулом
Size- число реплик данных для пула

Для пула rbd: ((100)x(9)x(0.7))/(3)= 210 (ближайшая степень двойки- 256)
Для пула cephfs_data: ((100)x(9)x(0.3))/(3)= 90 (ближайшая степень двойки- 128)
Для пула cephfs_metadata: 32 (стандарт для мета-пулов)

При этом в кластере работает pg autoscaler, который зарезал кол-во всех pg до 32
