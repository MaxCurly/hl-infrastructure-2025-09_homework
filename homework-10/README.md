...
1. Загрузка cloud-диска c с репозитория ubuntu
2. Создание ВМ с импортом загруженного cloud-диска (при развертывании ВМ отрабатывает cloud-init)

Инструкция по созданию ВМ:

1. Необходимо сделать экспорт учетных жанных для подключения к proxmox в переменные среды:
